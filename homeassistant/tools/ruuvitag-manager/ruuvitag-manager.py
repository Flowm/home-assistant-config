...
class RuuviTagManager:
    ...
    def _on_connect(self, client, userdata, flags, reason_code, properties):
        ...
        if reason_code == 0:
            ...
            logger.info("Subscribed to Home Assistant status messages")
        else:
    ...
    def _on_message(self, client, userdata, msg):
        ...
        try:
            if msg.topic == "homeassistant/status":
                ...
                if payload == "online":
                    logger.info("Home Assistant came online - resending discovery configs")
                    self._resend_all_configs()
    ...
    def _resend_all_configs(self) -> None:
        """Resend discovery configs for devices active in the last day"""
        current_time = time.time()
        one_day_ago = current_time - (24 * 60 * 60)  # 24 hours in seconds

        for address in self.configured_devices:
            last_update = self.last_update_times.get(address, 0)
            if last_update >= one_day_ago:
                logger.info(f"Resending config for active device {address}")
                self._publish_ha_config(address)
            else:
                logger.debug(f"Skipping config for inactive device {address} (last seen {(current_time - last_update) / 3600:.1f} hours ago)")
